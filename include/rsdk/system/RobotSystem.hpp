...
namespace rsdk
{
    ...
    {
    protected:
        // 子类实现时调用的注册函数, 如果plguin_hash不符合任何预定义的插件类型，则返回false
        template<class T, class G>
        bool registPlugin(const std::shared_ptr<G>& impl)
        {
            static_assert(std::is_base_of<rsdk::PluginBase, T>::value, "error!");
    ...
    private:
        // 注册插件
        bool _regist_plugin(size_t plugin_hash, const std::shared_ptr<PluginBase>& impl);

        // 尝试在系统中获取某种插件的具体实现
        // 如果没有则返回 nullptr
        std::shared_ptr<PluginBase> _getPlugin(size_t plugin_hash);
    ...
    public:
        ...
        struct SystemLinkResult
        {
            bool is_success;
            std::string detail;
        };
        
        virtual SystemLinkResult link(const SystemConfig& config) = 0;
